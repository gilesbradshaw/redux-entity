...
var getModule = function getModule(_ref) {
  ...
  var getJoinFilter = _ref$getJoinFilter === undefined ? function (_ref26) {
    var parentId = _ref26.parentId;
    ...
    return function (_ref4) {
      ...
      return function (actions, _ref5) {
        ...
        if (getLoadHasChanged({ loadDefaults: loadDefaults, getState: getState })) {
          return join(loadConfig)({ signalR: signalR, apiClient: apiClient }).switchMap(function (changes) {
  ...
  var orderData = function orderData(values, state) {
    var loadOrder = state.loadDefaults && state.loadDefaults.query && state.loadDefaults.query.order;

    if (loadOrder) {
      var _ret5 = function () {
        var back = loadOrder[0] === '-';
        var field = back ? loadOrder.substring(1) : loadOrder;
        var ret = values.map(function (value) {
          return value;
        }).filter(function (value) {
          return filter(state.loadDefaults, value);
        }).sort(function (a, b) {
  ...
  var ACTION_HANDLERS = (_ACTION_HANDLERS = {}, (0, _defineProperty3.default)(_ACTION_HANDLERS, ENTITIES_LOAD, function (state, action) {
    ...
    var data = state.data && state.data.Values ? [].concat((0, _toConsumableArray3.default)(state.data.Values)) : [];

    if (state.data && state.data.Values) {
      if (newIn) {
        var found = data.find(function (value) {
    ...
    var ret = (0, _extends28.default)({}, state, {
      rowVersions: (0, _extends28.default)({}, state.rowVersions, action.payload.value.RowVersion ? (0, _defineProperty3.default)({}, action.payload.value.RowVersion, true) : null),
    ...
    var data = state.data && state.data.Values ? [].concat((0, _toConsumableArray3.default)(state.data.Values)) : [];
    var newIn = filter(state.loadDefaults, action.payload.value);
    var found = data.find(function (state) {
      return state.Id === action.payload.value.Id;
    });
    var totalCountChange = newIn && !found ? 1 : !newIn && found ? -1 : 0;
